...
with 500k SNPs on a standard desktop machine.
...
information (e.g. Beta, Odds Ratio, signed summary statistics, Z-score*).
...
these file types. The package also gives the suer the flexibility to export the
reformatted file as tab-delimited, VCF or R native objects such as data.table,
GRanges or VRanges objects.
...
-   **allele_flip_check** Binary, should the allele columns be checked
    ...
    is TRUE. **allele_flip_drop** controls whether the Z-score value should be 
    flipped along with effect columns (e.g. Beta). Default is TRUE. 
...
See our publication for further discussion of these checks:
