test_that("Handle dup cols", {
    ...
    expect_equal(file.exists(reformatted$log_files$MungeSumstats_log_msg),TRUE)
    expect_equal(file.exists(reformatted$log_files$MungeSumstats_log_output),
                    TRUE)
    ...
    #should equal org
    writeLines(eduAttainOkbay,con = file)
    org_before_dup <- MungeSumstats::format_sumstats(file,ref_genome="GRCh37",
                                                  N_std=5,convert_n_int=FALSE,
                                                  on_ref_genome = FALSE,
                                                  strand_ambig_filter=FALSE,
                                                  bi_allelic_filter=FALSE,
