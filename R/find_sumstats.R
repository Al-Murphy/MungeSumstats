...
find_sumstats <- function(ids=NULL,
    ...
    if(any(!is.null(min_sample_size), !is.null(min_ncase), !is.null(min_ncontrol), !is.null(min_nsnp))){
        ...
        if(include_NAs){
            ...
            if(!is.null(min_nsnp)) metagwas <- subset(metagwas, nsnp>=min_nsnp | is.na(nsnp) | nsnp=="NA")
        }else {
    ...
    message("Found ",formatC(nrow(metagwas),big.mark = ",")," GWAS datasets matching search criteria across:",
            ...
            "\n   - ",formatC(length(unique(metagwas$population)),big.mark = ",")," population(s)",
            "\n   - ",formatC(length(unique(metagwas$category)),big.mark = ",")," category(ies)",
            "\n   - ",formatC(length(unique(metagwas$subcategory)),big.mark = ",")," subcategory(ies)",
            "\n   - ",formatC(length(unique(metagwas$pmid)),big.mark = ",")," publication(s)",
            "\n   - ",formatC(length(unique(metagwas$consortium)),big.mark = ",")," consortia(ium)",
