...
#' @keywords internal 
check_save_path <- function(save_path,log_folder,log_folder_ind,
                                write_vcf=FALSE){ 
    ...
    if(log_folder_ind && log_folder==tempdir()){
        message("******::NOTE::******\n",
                ...
                "- To keep it, change `log_folder` to an actual directory (e.g. log_foler='./').\n",
                "********************",
                "\n") 
    }
    ...
    if(write_vcf){ 
        ...
        if(any(endsWith(save_path,suffixes.vcf))){
            message("`write_vcf=FALSE` but `save_path` suggests VCF output ",
        ...
        #get extension of save path for log files
        if(!exists("extension"))#if output vcf, save log file type just .tsv.gz
            extension <- ".tsv.gz"
    }  
