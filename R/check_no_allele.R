...
check_no_allele <- function(sumstats_file, path, ref_genome, rsids){
  ...
  if(sum(c("A1","A2") %in% col_headers)<=1 & sum("SNP" %in% col_headers)==1){
    ...
    if(sum(c("A1","A2") %in% col_headers)==1){ #one allele missing
      ...
      rsids[sumstats_file,ss_A:=get(ssA)]
      ...
      if(more_matched_allele!=ssA){
        ...
        data.table::setnames(sumstats_file,
        ...
        effect_columns <- 
          effect_columns[effect_columns %in% names(sumstats_file)]
